...
PROJECT_DESCRIPTION = (
    '## To access DrinkJoy API endpoints: \n'
    '1. **Obtain a Bearer Token:** Send a `POST` request to `/users/token/` with your credentials. The server responds with an **access** and **refresh** tokens.\n'
    '2. **Use the Bearer Token:** Include the access token in the Authorization header of your requests to protected endpoints: `Authorization: Bearer <access_token>`.\n'
    '3. **Handle Unauthorized Access:** If the token is invalid or expired, the server responds with `401 Unauthorized`. Obtain a new token by repeating step 1.'
)
