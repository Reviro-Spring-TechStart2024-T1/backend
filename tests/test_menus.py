...
@pytest.mark.django_db
def test_get_menu_with_no_menu_items_as_customer(
    jwt_auth_api_client,
...
@pytest.mark.django_db
def test_get_menu_with_no_menu_items_as_partner(
    jwt_auth_api_client,
    create_menu_from_factory
):
    # given:
...
@pytest.mark.django_db
def test_get_menu_with_menu_items_as_partner(
    jwt_auth_api_client,
    create_num_of_menu_items_in_one_menu_from_outside_factory,
    create_menu_from_factory
):
    # given:
    client = jwt_auth_api_client(role='partner')
    menu = create_menu_from_factory
    create_num_of_menu_items_in_one_menu_from_outside_factory(menu, 3)
    # when:
    url = reverse('menu-detail', args=[menu.id])
    response = client.get(
        url
